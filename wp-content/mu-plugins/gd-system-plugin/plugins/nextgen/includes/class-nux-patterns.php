...
class NUX_Patterns {
	...
	public function load_remote_patterns( $settings ) {
		...
		$nux_api_endpoint = apply_filters( 'nextgen_nux_api_endpoint', 'https://wpnux.godaddy.com/v2/api' );
		$locale           = determine_locale();
		$transient_key    = self::TRANSIENT . '-' . md5( $nux_api_endpoint . $locale );
		$patterns         = get_transient( $transient_key );

		if ( false === $patterns ) {
			$response = wp_remote_get( add_query_arg( 'lang', $locale, $nux_api_endpoint . '/patterns' ) );
