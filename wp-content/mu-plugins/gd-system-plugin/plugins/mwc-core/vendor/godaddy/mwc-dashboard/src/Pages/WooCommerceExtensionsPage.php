...
class WooCommerceExtensionsPage extends WordPressAdminPage
{
    ...
    use IsSinglePageApplicationTrait {
        render as renderSinglePageApplicationContainer;
    }
    ...
    public function __construct()
    {
        ...
        // TODO: replace with mwcExtensionsClient if the variable name changes in the client {WV 2020-02-13}
        // https://github.com/gdcorp-partners/mwc-extensions-client/commit/a4b2b31a83efb28bbfa44c2768285653524bc5f6#r47077122
        $this->appHandle = 'mwcDashboardClient';
        $this->appSource = Configuration::get('mwc_extensions.client.source.url');
    ...
    public function enqueueAssets()
    {
        ...
        Enqueue::style()
            ...
            ->setSource(Configuration::get('mwc_extensions.assets.admin.url'))
            ->execute();

        Enqueue::script()
            ->setHandle("{$this->appHandle}-runtime")
            ->setSource(Configuration::get('mwc_extensions.client.runtime.url'))
            ->setDeferred(true)
            ->execute();

        Enqueue::script()
            ->setHandle("{$this->appHandle}-vendors")
            ->setSource(Configuration::get('mwc_extensions.client.vendors.url'))
            ->setDeferred(true)
            ->execute();
    ...
    {
        $this->renderSinglePageApplicationContainer();
    }
