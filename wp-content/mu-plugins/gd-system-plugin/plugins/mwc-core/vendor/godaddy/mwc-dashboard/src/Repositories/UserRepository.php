...
use GoDaddy\WordPress\MWC\Dashboard\Tests\Unit\Repositories\WP_User;
...
class UserRepository
{
     ...
     * @param \WP_User|null $user
     * @return string
     * @throws \GoDaddy\WordPress\MWC\Common\Exceptions\BaseExceptionHandler
     */
    public static function getPasswordResetUrl(\WP_User $user = null): string
    {
        ...
        if (is_wp_error($passwordResetKey)) {
            throw new BaseExceptionHandler($passwordResetKey->get_error_message());
        }
    ...
    {
        return (bool) (new MessagesOptedIn(WordPressRepository::getUser()->ID))->getUserMeta();
    }
}
